...
To use a paid LLM you need an API key. Below is a basic guide to getting keys from each provider, though steps may change as providers update their websites. **NEVER share your API key with anyone — keep it stored in a secure place.** Slag Legion does not proxy, mark up, or touch your API usage — your key connects directly to the provider.
