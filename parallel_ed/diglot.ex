...
include std/text.e
include llm.e       -- diglot: the readable LLM client (Ctrl-A translate key)
...
constant CONTROL_A = 1,   -- diglot: AI-translate this pane into the other one
		 CONTROL_B = 2,
		 CONTROL_C = 3,
		 CONTROL_D = 4,   -- alternate key for line-delete
		 CONTROL_E = 5,   -- diglot: run/execute the current column's file
		 CONTROL_L = 12,
...
-- cursor style: 
constant ED_CURSOR = BLOCK_CURSOR
...
constant SPECIAL_KEYS = {ESCAPE, BS, DELETE, XDELETE, PAGE_UP, PAGE_DOWN,
						 ...
						 CONTROL_R, CONTROL_L, CONTROL_E, CONTROL_A,
...
positive_int window_col     -- left screen column of current window's text
window_col = 1
positive_int window_width   -- number of text columns in current window
window_width = 80
...
-- clear one line within the current window's column
	position(window_base + sline, window_col)
	puts(SCREEN, BLANK_LINE[1..window_width])
end procedure

procedure ClearWindow()
-- clear the current window's column (text area only)
	for s = 1 to window_length do
		position(window_base + s, window_col)
		puts(SCREEN, BLANK_LINE[1..window_width])
	end for
end procedure

-- NOTE: hardware scroll() can only scroll full-width screen bands, so it
-- cannot scroll a single side-by-side column.  ScrollUp/ScrollDown instead
-- repaint the current window's visible text (DisplayWindow, defined below --
-- Euphoria resolves the forward reference).  Correct, just less clever.
procedure ScrollUp(positive_int top, positive_int bottom)
-- move text up one line: repaint the column
	DisplayWindow(b_line - s_line + 1, 1)
end procedure

procedure ScrollDown(positive_int top, positive_int bottom)
-- move text down one line: repaint the column
	DisplayWindow(b_line - s_line + 1, 1)
end procedure

procedure set_absolute_position(natural window_line, positive_int column)
-- move cursor to a line and an absolute (non-shifted) column within the
-- current window -- offset by the window's left screen column
	position(window_base + window_line, window_col + column - 1)
end procedure
...
-- if all_clear is TRUE then the screen area has already been cleared before getting here.
	...
	natural last, last_pos, color, len, vis
	...
	if last-s_shift > window_width then
		-- line extends beyond this window's right margin
		set_absolute_position(sline, window_width)
		text_color(BACKGROUND_COLOR)
		bk_color(NORMAL_COLOR)
		puts(SCREEN, this_line[window_width+s_shift])
		normal_video()
	elsif not all_clear then
		-- clear rest of this window's line (not the whole screen row)
		vis = last - s_shift
		if vis < 0 then
			vis = 0
		end if
		puts(SCREEN, BLANK_LINE[1..window_width - vis])
	end if
end procedure
...
-- window_line 0 is status line, window_line 1 is first line of text
	...
	if window_line = 0 then
		-- status line (offset into this window's column)
		position(window_base + window_line, window_col + column - 1)
	else
		s = s_shift
		while column-s_shift > window_width do
			s_shift += SHIFT
		end while
		...
		position(window_base + window_line, window_col + (column-s_shift) - 1)
	end if
end procedure
...
-- print message on this window's top (status) line
	sequence line
	set_position(0, 1)
	reverse_video()
	line = message & BLANK_LINE
	puts(SCREEN, line[1..window_width])
...
-- show F-key & file_name
	...
	set_top_line("")
	left = ' ' & f_key & file_name & comment
	text = "Esc for commands"
	-- keep the label inside this column so it can't spill into the divider
	-- or the neighbouring window's status line
	if length(left) > window_width - length(text) - 1 then
		left = left[1 .. window_width - length(text) - 1]
	end if
	puts(SCREEN, left)
	set_position(0, window_width - length(text))
	puts(SCREEN, text)
	normal_video()
end procedure
...
constant W_BUFFER_NUMBER = 1,
		 ...
		 W_WINDOW_LENGTH = 4,
		 W_B_LINE = 11,
		 W_WINDOW_COL = 17,      -- left screen column of this window
		 W_WINDOW_WIDTH = 18     -- text columns in this window
...
-- save current state variables for a window
	window_list[window_number] = {buffer_number, buffer_version, window_base,
								  ...
								  edit_tab_width, window_col, window_width}
...
-- restore state variables for a window
	...
	window_col = state[W_WINDOW_COL]
	window_width = state[W_WINDOW_WIDTH]
...
-- draw the vertical bars between side-by-side windows
	integer dcol
	if length(window_list) <= 1 then
		return
	end if
	normal_video()
	for i = 1 to length(window_list) - 1 do
		dcol = window_list[i][W_WINDOW_COL] + window_list[i][W_WINDOW_WIDTH]
		for row = 1 to screen_length do
			position(row, dcol)
			puts(SCREEN, '|')
		end for
	end for
...
-- lay the windows out side-by-side as vertical columns (parallel-Bible
-- style), each full height, separated by a one-column divider
	natural nwindows, cols, col, size

	nwindows = length(window_list)
	cols = screen_width - (nwindows - 1)   -- reserve one column per divider
	col = 1
	for i = 1 to length(window_list) do
		size = floor(cols / nwindows)
		window_list[i][W_WINDOW_BASE] = 1                    -- status on row 1
		window_list[i][W_WINDOW_LENGTH] = screen_length - 1  -- text below it
		window_list[i][W_WINDOW_COL] = col
		window_list[i][W_WINDOW_WIDTH] = size
		window_list[i][W_MY_BUFFER_VERSION] = -1 -- force redisplay
		col += size + 1                          -- skip the divider column
		nwindows -= 1
		cols -= size
	end for
end procedure
...
-- diglot: run the current column's file (Ctrl-E).
-- Euphoria files go through eui, Python (.py) through python3.
-- Output is CAPTURED to a file, then displayed by diglot itself and paused
-- with diglot's own wait_key.  Key point: nothing runs interactively in the
-- child, so no child process eats the first keypress -- the single key you
-- press to return goes straight to diglot.  That is what fixes the
-- press-it-twice problem the pager/shell-read approaches all had.
	sequence low, cmd
	object junk, line
	integer fn

	save_file(file_name)          -- run the very latest edits
	low = lower(file_name)
	if dot_e then
		cmd = "eui \"" & file_name & "\""
	elsif match(".py", low) then
		cmd = "python3 \"" & file_name & "\""
	else
		set_top_line("diglot can't run " & file_name & "  --  press a key")
		junk = wait_key()
		normal_video()
		goto_line(0, b_col)
		return
	end if
	-- run the program, redirecting all its output to a file (mode 2 = leave
	-- our screen alone; no shell prompt, no interactive child)
	system(cmd & " > diglot_run.out 2>&1", 2)
	-- now show that captured output on a cleared screen, using diglot's own
	-- puts (screen writes auto-flush, so it appears immediately)
	bk_color(BLACK)
	text_color(WHITE)
	clear_screen()
	fn = open("diglot_run.out", "r")
	if fn != -1 then
		line = gets(fn)
	...
	puts(SCREEN, "\n---- press any key to return to diglot ----")
	junk = wait_key()             -- ONE key, read by diglot itself
	while get_key() != -1 do
		-- drain any leftover bytes
	end while
	normal_video()
	clear_screen()                -- wipe the output before redrawing
	refresh_all_windows()
	goto_line(0, b_col)
end procedure

function has_prefix(sequence s, sequence p)
-- does s begin with p?
	if length(s) < length(p) then
		return 0
	end if
	return equal(s[1..length(p)], p)
end function

function strip_code_fences(sequence s)
-- drop any ``` markdown fence lines the model may have wrapped the code in
	sequence lines = split(s, '\n')
	sequence out = ""
	for i = 1 to length(lines) do
		if not has_prefix(trim(lines[i]), "```") then
			out &= lines[i] & '\n'
		end if
	end for
	return out
...
function lines_from_text(sequence txt)
-- turn a block of text into ed's buffer format: one line per element,
-- each ending in '\n'
	sequence parts = split(txt, '\n')
	sequence buf = {}
	for i = 1 to length(parts) do
		if i = length(parts) and length(parts[i]) = 0 then
			-- drop the empty tail split() leaves after a trailing newline
		else
...
procedure translate_current()
-- diglot: translate the current pane's code into the OTHER language using the
-- local model (Ctrl-A), and drop the result into the other pane.
-- Python <-> Euphoria.
	sequence src, src_lang, target_lang, prompt, result, answer
	object junk
	integer orig, other

	if length(window_list) < 2 then
		set_top_line("translate needs two panes -- split first (Esc c).  press a key")
		junk = wait_key()
		normal_video()
		goto_line(0, b_col)
		return
	...
	if dot_e then
		...
		src_lang = "Python"
		target_lang = "Euphoria"
	end if

	-- confirm: this replaces the OTHER pane's contents
	set_top_line(sprintf("translate %s -> %s into the other pane? (y/n) ",
	...
	-- gather this pane's code (each buffer line already ends in '\n')
	src = ""
	for i = 1 to length(buffer) do
		src &= buffer[i]
	end for

	set_top_line(sprintf("translating %s -> %s ...  (a few seconds)",
						 {src_lang, target_lang}))

	prompt = "Translate this " & src_lang & " code to " & target_lang &
			 ".  Output ONLY the translated " & target_lang &
			 " code, with no explanation and no markdown code fences.\n\n" & src
	result = strip_code_fences(chat(prompt))

	if length(trim(result)) = 0 then
		set_top_line("translate failed -- is Ollama running?  press a key")
		junk = wait_key()
		normal_video()
		goto_line(0, b_col)
		return
	...
	-- drop the translation into the OTHER pane's buffer (in memory; the user
	-- saves it themselves if they want to keep it)
	orig = window_number
	other = 1
	if orig = 1 then
		other = 2
	...
	switch_window(other)
	buffer = lines_from_text(result)
	buffer_multi = repeat(-1, length(buffer))
	set_modified()
	b_line = 1
	b_col = 1
	s_line = 1
	s_col = 1
	s_shift = 0
	goto_line(0, 1)          -- repaint the other pane with the translation
	switch_window(orig)
	set_top_line("")
	normal_video()
	goto_line(0, b_col)
...
-- edit the file in buffer
	...
	while not stop do
		...
		if good(key) then
			...
			elsif key = CONTROL_E then
				run_current()          -- diglot: run this column's file

			elsif key = CONTROL_A then
				translate_current()    -- diglot: AI-translate into the other pane
...
procedure open_into_current_window(sequence fname)
-- diglot: load a DIFFERENT file into the current window, giving it its own
-- fresh buffer.  Mirrors the per-file setup in ed().  Used after
-- clone_window() to fill the right-hand column with the second file.
	file_number file_no

	-- flush the (currently shared, cloned) buffer back into buffer_list so
	-- the left window keeps its contents before we detach with new_buffer()
	save_state()
...
--    eui ed.ex filename     - filename specified
	...
	set_position(1, 1)
	-- diglot: a second file on the command line opens in a right-hand column
	-- (parallel Bible: left = e.g. Python, right = e.g. Euphoria)
	if length(command) >= 4 and length(command[4]) > 0 then
		clone_window()                        -- make the second column
		open_into_current_window(command[4])  -- fill it with the second file
		switch_window(1)                      -- start focused on the left
	end if
	edit_file()
end procedure
...
-- startup and shutdown of ed()
	...
	-- Force a solid black text cursor via OSC 12 (ESC ] 12 ; #000000 BEL).
	-- The terminal keeps auto-inverting the glyph, so the character under
	-- the cursor stays readable.  (Change #000000 to e.g. #00FF00 for a
	-- bright cursor instead.)
	puts(SCREEN, 27 & "]12;#000000" & 7)

	-- Use the terminal's ALTERNATE screen buffer (like vim/less) so running
	-- programs and redraws don't pile up copies in the scrollback.
	puts(SCREEN, 27 & "[?1049h")
	clear_screen()
	...
	-- restore the terminal's default cursor color (OSC 112), then leave the
	-- alternate screen so the user's normal terminal + scrollback come back intact
	puts(SCREEN, 27 & "]112" & 7)
	clear_screen()
	puts(SCREEN, 27 & "[?1049l")
	ifdef UNIX then
		free_console()
	end ifdef
end procedure
