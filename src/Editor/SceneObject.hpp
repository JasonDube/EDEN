...
public:
    ...
    // Per-agent on/off switch (EDEN OS). Off by default: agents start inert and
    // are activated one at a time via the CLI (activate_bot). Drives the HEIDIC
    // agent script's facing (agent_online()) and gates chat.
