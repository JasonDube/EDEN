...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HEIDIC()
{
	// EDEN's HEIDIC game-scripting language. C-like syntax, so it reuses the
	// C-style tokenizer (strings, numbers, // comments, identifiers).
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		static const char* const keywords[] = {
			...
			"return", "break", "continue", "extern", "struct", "enum", "impl",
			"import", "include", "true", "false", "null", "and", "or", "not",
			"entity", "hot",
			"void", "bool", "string", "str",
		...
		static const char* const identifiers[] = {
			"self_translate", "self_set_position", "self_get_x", "self_get_y", "self_get_z",
			"self_rotate_y", "self_set_rotation", "self_play_anim",
			"self_dist_to_player", "self_face_player", "self_move_toward_player",
			"self_move_forward", "self_move_right", "self_ground_y", "self_snap_to_ground",
			"input_move_x", "input_move_z", "input_jump", "input_run", "mouse_dx", "mouse_dy",
			"print"
		};
		for (auto& k : identifiers)
		{
			Identifier id;
			id.mDeclaration = "HEIDIC API";
			langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
		}
