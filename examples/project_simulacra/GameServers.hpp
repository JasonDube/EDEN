...
public:
    ...
    struct Entry {
        ...
        int dependsOn = -1;
        bool reclaimPort = false;   // if the port is already held (stale orphan), kill it and fork fresh
    ...
    explicit GameServers(const std::string& projectRoot) {
        ...
        ai.reclaimPort = true;   // a leftover backend on 8080 has a broken stdout pipe — kill & refork
    ...
    // The model the player is actually talking to right now ("" until fetched from the
    // backend). Tracks provider/model switches made in the panel, so the game can adopt
    // the new identity the moment it changes.
    std::string currentModelName() const {
        if (!m_modelsFetched) return "";
        if (m_provider == "grok")     return m_grokModel;
        if (m_provider == "claude")   return m_claudeModel;
        if (m_provider == "deepseek") return m_deepseekModel;
    ...
    void start(size_t index) {
        ...
        if (isPortOpen(srv.port)) {
            if (srv.reclaimPort) {
                // A leftover process holds our port. If it's an orphan from a prior game its
                // stdout pipe is broken and every request 500s — so kill it and fork fresh.
                std::string cmd = "fuser -k " + std::to_string(srv.port) + "/tcp >/dev/null 2>&1";
                std::system(cmd.c_str());
                usleep(500000);   // let it die and release the port before we bind
                srv.log.push_back("[servers] Reclaimed port " + std::to_string(srv.port) + " (killed stale backend).");
            } else {
                srv.status = Entry::Running;   // shared service (ollama) — reuse it
    ...
    // Apply a saved "start each session with this" choice: point at the provider, and for
    // Ollama the specific model. Backend confirms asynchronously, same as a manual switch.
